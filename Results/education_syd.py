...
with open("syd_education_stats.json", "r") as mel_e:
    mel_education = json.load(mel_e)
with open("nsw_income.json", "r") as v2:
...
while i < len(mel_education):
    if (mel_education[i]['income'] <= 40000 and mel_education[i]['income'] >= 90000 and mel_education[i]['income'] == 0) :
       del mel_education[i]
    i += 1
...
X_prime = np.linspace(25000, 130000, 10)[:,np.newaxis]
